-- Honour a manually-pinned MD+1 / MD+2 focus (the split Week Setup recovery options) the
-- same way as MD-5 / MD+3: match the md-day token in planned_focus FIRST, before the
-- auto post-match derivation. Only the top of the md_day CASE changes.
...
WITH d AS (
  ...
  SELECT d.player_id,
    ...
    CASE
      ...
      WHEN COALESCE(d.planned_focus, ''::text) ~~* '%MD+2%'::text THEN 'MD+2'::text
      WHEN COALESCE(d.planned_focus, ''::text) ~~* '%MD+1%'::text THEN 'MD+1'::text

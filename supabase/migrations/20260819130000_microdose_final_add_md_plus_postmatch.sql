-- Auto-label the post-match recovery days MD+1 / MD+2 / MD+3.
-- A day is MD+N when it is the N-th day (1..3) after the last match AND still more than 5
-- days before the next match — so the pre-match MD-5..MD-1 build-up window wins once the
-- next game is close (this keeps a 5-days-out FORCE day = MD-5, not MD+3). Beyond MD+3 or
-- inside the 5-day pre-match window it falls through to the existing focus-based derivation.

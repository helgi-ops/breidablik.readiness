...
import { runRpeEmailReminders } from "@/lib/reminders/emailReminders";
...
export async function POST(req: Request) {
  try {
    ...
    const result = await runRpeEmailReminders(sb, {
      dateKey,
      timeZone,
      teamId,
    });

    return NextResponse.json({ ok: true, dateKey, timeZone, teamId, result });
  } catch (error: unknown) {

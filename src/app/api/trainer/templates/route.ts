...
export async function GET(req: Request) {
  try {
    ...
    const { data: templates, error: err } = await sb
      ...
      .select(
        "id, plan_name, plan_type, duration_weeks, sessions_per_week, readiness_enabled, deload_volume_pct, deload_intensity_pct, notes, is_public, created_at, updated_at"
      )
...
export async function POST(req: Request) {
  try {
    ...
    if (!body.planName || !body.planType || !body.durationWeeks || !body.sessionsPerWeek) {
      return NextResponse.json(
        { error: "Missing required fields: planName, planType, durationWeeks, sessionsPerWeek" },
        { status: 400 }
      );
    }
    ...
    const { data: template, error: err } = await sb
      ...
      .insert([
        {
          ...
          readiness_red_action: body.readinessRedAction ?? "recovery",
          readiness_yellow_action: body.readinessYellowAction ?? "deload",
          deload_volume_pct: body.deloadVolumePct ?? 70,
          deload_intensity_pct: body.deloadIntensityPct ?? 85,
      ...
      .select(
        "id, plan_name, plan_type, duration_weeks, sessions_per_week, readiness_enabled, deload_volume_pct, deload_intensity_pct, structure, notes, is_public, created_at, updated_at"
      )
      .single();

...
export async function POST(req: NextRequest) {
  ...
  if (H.some((h) => SB_ANY.includes(h)) && hasH("Team Name")) {
    return NextResponse.json({ ok: false, error: "This is a StatsBomb season Team Stats export (Team Name + League Average). Upload it on Opponent Analysis — this page takes the per-player Squad or Player Match Stats export." }, { status: 400 });
  }

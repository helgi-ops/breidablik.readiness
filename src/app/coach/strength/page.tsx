              ...
              <div className="mt-2 rounded-md border border-slate-200 bg-slate-50/70 p-3">
                ...
                {structureOptions.length > 0 && (
                  <div className="mt-4 border-t border-slate-200 pt-3">
                    ...
                    <p className="mb-2 text-[11px] leading-relaxed text-slate-600">
                      {t(
                        "Tie a training method to each day — the individualised (and auto) session lays that method out from your palette. MD-4 / MD-3 offer strength & power methods; MD-2 / MD-1 offer velocity/explosive methods (power contrast, potentiation cluster) that stay light + fast near the match. The recovery side rebuilds progressively: MD+1 adapts to each player's match minutes automatically (60+ min → protective isometrics for the legs plus fresh upper-body strength; under 30 min or a DNP → a real strength session for the missed load), so it isn't a method you pick here. MD+2 (moderate) and MD+3 (fuller) add a rebuild session only when you choose a method (Default = rest). Leave a day on Default to keep its built-in structure.",
                        "Tengdu æfingaaðferð við hvern dag — einstaklingsmiðaða (og sjálfvirka) æfingin raðar þeirri aðferð úr palette-inu þínu. MD-4 / MD-3 bjóða styrk- og afl-aðferðir; MD-2 / MD-1 bjóða hraða-/sprengikrafts-aðferðir (afl-contrast, potentiation cluster) sem haldast léttar + hraðar nálægt leik. Batahliðin byggist upp stig af stigi: MD+1 aðlagast leikmínútum hvers leikmanns sjálfkrafa (60+ mín → verndandi ísómetría fyrir fæturna auk óþreytts efri-líkama styrks; undir 30 mín eða DNP → alvöru styrktaræfing fyrir álagið sem tapaðist), svo það er ekki aðferð sem þú velur hér. MD+2 (hóflegt) og MD+3 (fyllra) bæta við æfingu aðeins þegar þú velur aðferð (Sjálfgefið = hvíld). Skildu dag eftir á Sjálfgefið til að halda innbyggðu uppsetningunni.",
                      )}
                    </p>

...
export default function WeekSetupPage() {
  ...
  // ✅ setMatch: auto-align weekStart to Monday of match week
  ...
  function applyWeekType(next: WeekType) {
    ...
    setMatches((prev) => {
      if (next === "ONE_MATCH") {
        return [prev[0] ?? { match_id: "M1", date: "", kickoff_time: "", home_away: "H" }];
      }
  ...
  // ✅ Week mapping: mán→sun; uses next match in week for MD-*, prev match in week for MD+*
  const dayEdits = useMemo(() => {
    const matchInWeek = matches
      .map((m, idx) => ({
        idx,
    ...
    <div className="mx-auto w-full max-w-5xl p-4 md:p-6">
        ...
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Vikan er mánudagur → sunnudagur. MD-talning er reiknuð frá næsta leik í vikunni (MD-*) og síðasta leik í vikunni
              (MD+*).
            </p>
          </div>
      ...
      <Card className="mt-4">
        <CardHeader>
          ...
          <CardDescription>Settu inn leikdag(a). Ef match date er valinn hoppar weekStart á rétta viku (mán→sun).</CardDescription>
        </CardHeader>
        <CardContent className="grid gap-3">
          {matches.map((m, idx) => (
            <div
              key={idx}
              className="grid gap-2 rounded-xl border p-3 md:grid-cols-[110px_170px_1fr_90px] md:items-center"
            >
      ...
      <Card className="mt-4">
        <CardHeader>
          ...
          <CardDescription>MD mapping innan vikunnar (mán→sun). Hover fyrir notes.</CardDescription>
        </CardHeader>
        <CardContent>
          ...
          <div className="mt-4 text-xs text-muted-foreground">
            Ef leikur er á föstudegi: mán=MD-4, þri=MD-3, mið=MD-2, fim=MD-1, fös=MD, lau=MD+1, sun=OFF.
          </div>
        </CardContent>
      </Card>
    </div>

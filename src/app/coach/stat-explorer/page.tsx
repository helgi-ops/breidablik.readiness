...
/**
 * Stat Explorer — pick a window (last 5 / 10 / all), a side of the game and a metric, and see
 * which players are producing the best numbers. Layered read: a one-line verdict (who leads) →
 * ranked bars → a full "show details" table (games / minutes for confidence). Reads
 * /api/coach/stat-explorer + ranks client-side with the shared pure engine. Descriptive football
 * context — never touches the readiness colour. Bilingual EN/IS.
 */
...
import {
  METRICS, GROUP_LABEL, rankLeaderboard,
  type PlayerAgg, type GroupKey, type Mode, type Line, type Bi,
...
type Resp = { ok: boolean; hasData?: boolean; window?: number | "all"; matchDates?: string[]; players?: PlayerAgg[]; catalog?: CatalogEntry[]; error?: string };

const GROUP_ORDER: GroupKey[] = ["attacking", "defensive", "buildup", "setpiece", "gk"];
...
const fmt = (v: number | null, agg: "sum" | "mean"): string =>
  v == null ? "–" : agg === "mean" ? `${v.toFixed(0)}%` : Math.abs(v) >= 10 || Number.isInteger(v) ? v.toFixed(v < 100 && !Number.isInteger(v) ? 1 : 0) : v.toFixed(2);
...
export default function StatExplorerPage() {
  ...
  React.useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const res = await fetch(`/api/coach/stat-explorer?window=${win}`, { cache: "no-store", headers: { Authorization: `Bearer ${t}` } });
  ...
  // Keep the selected metric inside the chosen group.
  const groupMetrics = React.useMemo(() => METRICS.filter((m) => m.group === group), [group]);
  React.useEffect(() => {
    if (!groupMetrics.some((m) => m.key === metricKey)) setMetricKey(groupMetrics[0]?.key ?? "goals");
  }, [group, groupMetrics, metricKey]);

  const spec = METRICS.find((m) => m.key === metricKey);
  // %s can't be per-90/total; force per-game display and disable the toggle.
  const effMode: Mode = spec?.agg === "mean" ? "perGame" : mode;

  const board = React.useMemo(() => {
    if (!data?.players) return null;
    return rankLeaderboard(data.players, { metricKey, mode: effMode, minGames, line: line === "ALL" ? null : line });
  }, [data, metricKey, effMode, minGames, line]);
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-6">
      ...
      <PagePurpose en="pick a side of the game and a window, and see which players are producing the best numbers over the last few matches." is="velja hlið leiksins og glugga, og sjá hvaða leikmenn skila bestu tölunum síðustu leiki." />
      ...
      <div className="mt-4 space-y-3 rounded-2xl border border-slate-200 bg-white p-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {GROUP_ORDER.map((g) => (
            <button key={g} onClick={() => setGroup(g)} className={`rounded-full px-3 py-1 text-[12px] font-semibold ${group === g ? "bg-[#14181c] text-white" : "bg-slate-100 text-slate-600 hover:bg-slate-200"}`}>
              {L(GROUP_LABEL[g])}
            </button>
          ))}
        </div>
          ...
          {spec?.agg === "sum" ? (
            <div className="inline-flex overflow-hidden rounded-lg border border-slate-200">
              {(["perGame", "total", "per90"] as Mode[]).map((mo) => (
                <button key={mo} onClick={() => setMode(mo)} disabled={mo === "per90" && !spec.per90}
                  className={`px-2.5 py-1 text-[12px] font-semibold ${mode === mo ? "bg-[#2740e6] text-white" : "bg-white text-slate-600 hover:bg-slate-50"} disabled:opacity-30`}>
                  {L(modeLabel[mo])}
                </button>
              ))}
            </div>
          ) : (
            <span className="rounded-lg bg-slate-100 px-2.5 py-1 text-[12px] font-medium text-slate-500">{is ? "vegið meðaltal" : "weighted avg"}</span>
          )}
      ...
      {/* ── Result ─────────────────────────────────────────────── */}
      {state === "ready" && board && spec ? (
        <div className="mt-6 space-y-4">
          {/* (0) Verdict */}
          <div className="rounded-2xl border border-slate-200 bg-white p-4">
            <div className="text-[11px] font-semibold uppercase tracking-wide text-slate-400">
              {windowLabel} · {L(GROUP_LABEL[group])} · {L(spec.label)}{spec.agg === "sum" ? ` (${L(modeLabel[effMode])})` : ""}
            </div>
            {top ? (
            ...
            {board.rows.length >= 2 && top ? (
              <p className="mt-0.5 text-[12.5px] text-slate-500">
                {is ? "Næstir: " : "Then: "}{board.rows.slice(1, 3).map((r) => `${r.name} (${fmt(r.value, spec.agg)})`).join(is ? ", " : ", ")}
              </p>
            ) : null}
          ...
          {board.rows.length > 0 ? (
            <div className="rounded-2xl border border-slate-200 bg-white p-4">
                ...
                {board.rows.slice(0, 12).map((r, i) => (
                  <div key={r.playerId} className="flex items-center gap-2">
                    ...
                    <div className="relative h-4 flex-1 overflow-hidden rounded bg-slate-100">
                      <div className="h-full rounded bg-[#2740e6]/85" style={{ width: `${max > 0 && r.value != null ? Math.max(2, (r.value / max) * 100) : 0}%` }} />
                    </div>
              ...
              {/* (2) Details */}
              <button onClick={() => setShowDetails((v) => !v)} className="mt-3 text-[12px] font-medium text-[#2740e6] hover:underline">
                {showDetails ? (is ? "Fela nánar" : "Hide details") : (is ? "Sýna nánar (heild, á leik, mínútur)" : "Show details (total, per-game, minutes)")}
              </button>
              {showDetails ? (
                <div className="mt-2 overflow-x-auto">
                  <table className="w-full text-[12px]">
                    <thead>
                      <tr className="text-left text-slate-400">
                        <th className="py-1 pr-3 font-medium">{is ? "Leikmaður" : "Player"}</th>
                        <th className="pr-3 font-medium">{is ? "Staða" : "Pos"}</th>
                        <th className="pr-3 text-right font-medium">{is ? "Leikir" : "Games"}</th>
                        <th className="pr-3 text-right font-medium">{is ? "Mínútur" : "Minutes"}</th>
                        <th className="pr-3 text-right font-medium">{is ? "Samtals" : "Total"}</th>
                        <th className="pr-3 text-right font-medium">{is ? "Á leik" : "Per game"}</th>
                      </tr>
                    </thead>
                    <tbody>
                      {board.rows.map((r) => (
                        <tr key={r.playerId} className="border-t border-slate-100">
                          <td className="py-1.5 pr-3 text-slate-700">{r.name}</td>
                          <td className="pr-3 text-slate-500">{r.position ?? "–"}</td>
                          <td className="pr-3 text-right tabular-nums text-slate-600">{r.games}</td>
                          <td className="pr-3 text-right tabular-nums text-slate-500">{r.minutes > 0 ? Math.round(r.minutes) : "–"}</td>
                          <td className="pr-3 text-right tabular-nums text-slate-700">{fmt(r.total, spec.agg)}</td>
                          <td className="pr-3 text-right tabular-nums font-semibold text-slate-900">{fmt(r.perGame, spec.agg)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                  <p className="mt-2 text-[11px] leading-relaxed text-slate-400">
                    {is
                      ? "Lýsandi tölfræði úr StatsBomb per-leik gögnum — snertir aldrei readiness-litinn. „Á leik“ = samtals ÷ leikir. Per-90 birtist þegar mínútur eru skráðar (Squad-innflutningur ber þær)."
                      : "Descriptive stats from StatsBomb per-match data — never touches the readiness colour. “Per game” = total ÷ games. Per-90 appears once minutes are recorded (the Squad import carries them)."}
                  </p>
                </div>
              ) : null}
            </div>

...
export default function MyExercisesPage() {
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-4 p-4">
          ...
          <ul className="divide-y divide-slate-100">
            {visible.map((e) => (
              <li
                ...
                className="flex cursor-pointer items-center justify-between gap-3 px-4 py-2 hover:bg-slate-50"
              >
                <div className="min-w-0">
                  ...
                  <div className="text-[11px] text-slate-400">
                    ...
                    {e.video_url ? " · ▶ video" : ""}{e.description ? " · ⓘ" : ""}
                  </div>
                </div>
                {e.editable && (
                  <div className="flex shrink-0 items-center gap-2">
                    <button type="button" onClick={(ev) => { ev.stopPropagation(); edit(e); }} className="rounded-md border border-slate-200 px-2 py-1 text-xs text-slate-600 hover:bg-slate-50">{is ? "Breyta" : "Edit"}</button>
                    <button type="button" onClick={(ev) => { ev.stopPropagation(); void remove(e); }} className="rounded-md border border-red-200 px-2 py-1 text-xs text-red-600 hover:bg-red-50">{is ? "Eyða" : "Delete"}</button>
                  </div>
                )}
              </li>
            ))}
      ...
      {/* Detail modal — description + video for the clicked exercise. */}
      {selected && (
        <div
          ...
          >
            <div className="mb-2 flex items-start justify-between gap-3">
              <div className="min-w-0">
                ...
                {selected.name_is && selected.name_is !== selected.name && (
                  <div className="text-xs text-slate-400">{selected.name_is}</div>
                )}
                <div className="mt-1 flex flex-wrap items-center gap-1.5">
                  <span className={`rounded px-1.5 py-0.5 text-[9px] font-medium ${selected.editable ? "bg-indigo-100 text-indigo-700" : "bg-slate-100 text-slate-500"}`}>
                    {selected.editable ? (is ? "Mín æfing" : "My exercise") : (is ? "🔒 Kerfis-æfing" : "🔒 System exercise")}
                  </span>
                  {[selected.exercise_type, selected.movement_family, selected.equipment].filter(Boolean).map((t) => (
                    <span key={t as string} className="rounded bg-slate-100 px-1.5 py-0.5 text-[10px] text-slate-500">{t}</span>
                  ))}
                </div>
              </div>
              <button type="button" onClick={() => setSelected(null)} className="shrink-0 text-slate-400 hover:text-slate-600" aria-label="Close">✕</button>
            </div>
            ...
            {selected.video_url && /^https?:\/\//.test(selected.video_url) && (
              <a
              ...
              >
                ▶ {is ? "Horfa á myndband" : "Watch video"}
              </a>
            )}

            {selected.editable && (
              <div className="mt-4 flex items-center gap-2 border-t border-slate-100 pt-3">
                <button type="button" onClick={() => { const e = selected; setSelected(null); edit(e); }} className="rounded-md border border-slate-200 px-3 py-1.5 text-sm text-slate-600 hover:bg-slate-50">{is ? "Breyta" : "Edit"}</button>
                <button type="button" onClick={() => { const e = selected; setSelected(null); void remove(e); }} className="rounded-md border border-red-200 px-3 py-1.5 text-sm text-red-600 hover:bg-red-50">{is ? "Eyða" : "Delete"}</button>
              </div>
            )}
          </div>
        </div>

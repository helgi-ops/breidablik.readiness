...
export function getExerciseRecommendation(input: ExerciseRecommendationInput): ExerciseRecommendationResult {
  ...
  if (group === "EXPLOSIVE_ACCESSORY") {
    if (
      ...
      playerText = "Í dag veljum við léttari útgáfu til að minnka álag eftir leik.";
      ...
      if (input.scheduleCongestionFlag) {
        ...
        playerText = "Þétt leikáætlun í gangi — við drögum úr CNS álagi og veljum Mid-Thigh Pull í dag.";
      } else {
  ...
  if (group === "UNILATERAL_STRENGTH_ACCESSORY") {
    if (
      ...
      playerText = "Í dag veljum við Split Stance Trap Bar Deadlift til að hlífa bakhluta.";
      ...
      playerText = "Í dag veljum við Isometric Split Squat Hold til að hlífa lærismöglum.";
    } else if (input.readinessState === "YELLOW" || input.riskState === "MODERATE") {
      if (input.unilateralDeficitFlag) {
        ...
        playerText = "Við þjálfum RFESS sérstaklega í dag til að jafna mun á milli hliða.";
      } else {
        recommendedExerciseId = "RFESS";

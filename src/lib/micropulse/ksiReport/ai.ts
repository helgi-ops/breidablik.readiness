...
You are given a JSON object of rule-computed numbers: the window, session count, and his accrued GPS/IMA load totals (distance, high-speed running, sprint distance, top speed, accelerations, decelerations, PlayerLoad, IMA high-speed).

Write STRICTLY from these numbers — never invent values, tests, minutes or injuries. Describe ONLY what HE has been doing, using HIS OWN absolute figures: his training/match volume, how much high-speed and sprint running he has done, his mechanical load (accel/decel) and his top speed. Do NOT compare him to teammates, the squad, a league, or any benchmark, and do not use percentiles or rankings — the federation wants his own numbers, not a comparison. Write ONLY about his load figures — do NOT mention injuries, prevention/strength programmes, staff, or who oversees his training; those are reported in their own dedicated sections and must not be restated here (restating them risks contradicting the coach's own notes). Cite actual numbers with units. Neutral, factual, respectful. Make NO claim about readiness, availability, selection or injury risk — this only describes his recent load.
...
export async function buildKsiAiSummary(facts: KsiAiFacts, lang: "EN" | "IS"): Promise<KsiAiSummary> {
  ...
  const res = await fetch("https://api.anthropic.com/v1/messages", {
    ...
    body: JSON.stringify({
      ...
      system: AI_SYSTEM,
      // Send only his OWN absolute load numbers. Never the squad percentiles (radar) — so the
      // model cannot compare him to the team — and never the injury/programme notes — so the
      // narrative cannot drift from, or contradict, the coach's own note fields (those are
      // reported verbatim in their own sections). KSÍ wants his figures, not a ranking.
      messages: [{ role: "user", content: `Write in ${lang === "IS" ? "Icelandic" : "English"}. Player load data (JSON):\n\n${JSON.stringify({ name: facts.name, window: facts.window, load: facts.load })}` }],
    }),
  });
  if (!res.ok) {

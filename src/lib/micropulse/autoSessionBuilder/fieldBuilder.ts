...
/** Builds a field-oriented draft and applies deterministic exposure protections. */
export function buildFieldSessionDraft(input: SessionBuildInput): SessionDraft {
  ...
  const template = getMatchdayAwareTemplate({ sessionType: "FIELD", dayType: input.dayType });

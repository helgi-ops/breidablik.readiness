...
function fixture(): RttSession[] {
  ...
  const injured = ["2026-06-15", "2026-06-17", "2026-06-22", "2026-06-24"].map((d) =>
    session(d, { injured: true, load: 300, distance: 3500, hsr: 200, sprint: 0, stride: 15, strideTop: 25, accel: 10, decel: 8, decelHigh: 1, cod: 0, codLeft: 0, codRight: 0, topSpeed: 20 }),
  );
...
test("injuryRiskProfile maps tissue → key re-injury qualities (incl. high-intensity braking)", () => {
  assert.deepEqual(new Set(injuryRiskProfile(["hamstring"]).riskQualities), new Set(["hsr", "sprint", "strideTop", "decel", "decelHigh"]));

 ...
 * typical value vs the squad, which surfaces his movement SIGNATURE — a centre-back
 * reads high-decel / low-sprint) and whether the signal is IMA-fidelity (hard
 * accel/decel, CoD) or GPS. The signed z ranks: a quality he does a lot of for his
 * role rises; one he barely does (a centre-back's sprinting) drops below the floor.
 * Every player still gets his top few. Branches on signals PRESENT, never tier name:
...
export function computeTrainingRead(input: TrainingReadInput): PlayerTrainingRead {
  ...
  for (const q of QUALITY_KEYS) {
    ...
    // SIGNED z (vs squad norm): high-for-the-team raises the score; a quality he
    // barely does (a centre-back's sprinting, strongly negative z) drops below the
    // SCORE_FLOOR. Every player still gets his top few — his movement signature.
    const z = sig!.z ?? 0;

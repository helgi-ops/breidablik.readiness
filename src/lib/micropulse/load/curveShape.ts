...
const CAVEAT: Bi = {
  en: "Curve shape reads the power curve from the Catapult peak-period export — how much of his short-burst peak (≤30 s) he holds over minutes (retention). Explosive = high short peak that fades fast; Engine = holds a high fraction over long windows; Under-conditioned = a low ceiling at every window vs his peers. The retention thresholds are provisional until calibrated on real curves, and it needs the peak-period feed (it can't be read from session summaries). Descriptive — it never changes the readiness verdict or the daily plan.",
  is: "Kúrfu-lögun les afl-kúrfuna úr Catapult peak-period útflutningi — hversu miklu af skammtíma-hámarki (≤30 s) hann heldur yfir mínútur (retention). Explosive = hátt skammtíma-hámark sem dvínar hratt; Engine = heldur háu hlutfalli yfir langa glugga; Under-conditioned = lágt þak í öllum gluggum m.v. jafningja. Retention-mörkin eru til bráðabirgða þar til þau eru kvörðuð á raunverulegum kúrfum, og þetta þarf peak-period strauminn (ekki lesanlegt úr lotu-samantektum). Lýsandi — breytir aldrei readiness-dómnum eða dagsáætluninni.",
};

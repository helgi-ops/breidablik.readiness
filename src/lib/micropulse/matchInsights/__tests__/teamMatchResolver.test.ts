...
describe("buildResolvedFromSbRows (StatsBomb per-match resolver)", () => {
  const rows = [
    { match_date: "2026-08-04", opponent: "Thor Akureyri", goals: 0, goals_against: 1, xg: 1.61, xg_against: 1.62, shots: 14, shots_against: 13, possession_proxy_pct: 60.8, passes: 500, passing_pct: 82.6, crosses: 17, box_touches: 39, obv: 0.76, opposition_obv: 2.19, set_piece_xg: 1.1, opp_set_piece_xg: 0.92, pressures: 106, deep_progressions: 46, passes_into_box: 36, updated_at: "2026-08-05T00:00:00Z" },
  ...
  it("builds provider-agnostic maps + StatsBomb extras", () => {
    ...
    // passing_pct (completion %) → derived accurate-pass count so the shared Pass%
    // column (passes_accurate/passes) fills: 500 × 82.6% = 413.
    expect(r.own.get("2026-08-04")?.passes).toBe(500);
    expect(r.own.get("2026-08-04")?.passes_accurate).toBe(413);

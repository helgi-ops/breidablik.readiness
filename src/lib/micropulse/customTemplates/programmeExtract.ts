...
- BLOCK-LEVEL REST & ROUNDS → dedicated fields, taken OUT of the exercise text. If a block-wide "rest between sets" value or a rounds/circuit count is shared by the WHOLE block, put the between-sets rest in the block's "rest_between_sets" field and ONLY the rounds count — written as "N rounds" and nothing else — in the block's "rounds" field (e.g. rest_between_sets: "60–90 sec", rounds: "2 rounds"). DROP any "rest between rounds" duration entirely; do NOT append it to the rounds field. Keep each exercise's own sets×reps ON its line — including a cluster's shared set count, e.g. "1a Deadlift · 6 sets × 1 rep". Remove the extracted rest / rounds phrases from the exercise lines so they are not duplicated. Only keep a rest value on an exercise line when it differs per exercise. Leave "rest_between_sets"/"rounds" null when the block has no shared value.

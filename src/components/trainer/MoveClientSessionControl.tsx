...
/**
 * MoveClientSessionControl — the trainer's control to move a client's prescribed
 * session from one day to another (the coach-side parallel to the client's
 * "move to another day"). Pick the day that has the session and the new day.
 * Lists active moves with undo. Relocates just that one session.
 */
...
export default function MoveClientSessionControl({ clientId, lang }: { clientId: string; lang: "EN" | "IS" }) {
  const is = lang === "IS";
  ...
  const load = useCallback(async () => {
    try {
      const res = await fetch(`/api/trainer/client/${clientId}/session-reschedule`, { headers: await authHeader() });
  ...
  const move = async () => {
    ...
    try {
      const res = await fetch(`/api/trainer/client/${clientId}/session-reschedule`, {
        method: "POST",
      ...
      setFrom(""); setTo(""); setOpen(false); load();
  ...
  const undo = async (fromDate: string) => {
    setBusy(true);
    try {
      const res = await fetch(`/api/trainer/client/${clientId}/session-reschedule?from_date=${fromDate}`, { method: "DELETE", headers: await authHeader() });
      if (res.ok) load();
    } catch { /* ignore */ }
    finally { setBusy(false); }
  };
    ...
    <div className="rounded-lg border border-slate-200 bg-white px-3 py-2">
      <div className="flex items-center justify-between gap-2">
        <div>
          ...
          <div className="text-[11px] text-slate-500">{is ? "Færa æfingu viðskiptavinar á annan dag." : "Relocate a client's session to another day."}</div>
        </div>
      ...
      {open && (
        <div className="mt-2 flex flex-wrap items-end gap-2">
          <label className="text-[10px] text-slate-500">
            <span className="block">{is ? "Frá degi" : "From day"}</span>
          ...
          <label className="text-[10px] text-slate-500">
            <span className="block">{is ? "Á dag" : "To day"}</span>
            <input type="date" value={to} onChange={(e) => setTo(e.target.value)} className="mt-0.5 rounded-md border border-slate-300 px-2 py-1 text-sm" />
          </label>

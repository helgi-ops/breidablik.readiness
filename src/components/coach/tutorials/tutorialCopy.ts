...
export const TUTORIALS: Record<TutorialSlug, Tutorial> = {
  ...
  "progressive-overload": {
    ...
    videoAspectPaddingTop: "75%",
    intro: {
      en: "Progressive Overload now lives in the Periodization Hub: the Meso Cycle ramp is engine-driven (volume climbs faster than HSR/sprint, each ceilinged at match), and the per-player build-up plan — a safe multi-week ramp from current baseline toward match demand — is on the Hub's Players tab. Volume ramps faster than high-speed/sprint (the hamstring risk), each KPI ceilings at match load, and weeks are held so acute load doesn't spike (ACWR is a contested reference only). Open the Periodization Hub → Players → Build-up plan.",
      is: "Progressive Overload er núna í Tímabilsskipulaginu: Mesó-lotu rampinn er vél-drifinn (magn hækkar hraðar en háhraði/sprettur, hvert við leikþak), og uppbyggingar-áætlun per leikmann — öruggur fjölvikna rampi frá núverandi grunnlínu að leikkröfu — er á Leikmenn-flipa hubbsins. Magn rampar hraðar en háhraðahlaup/spretti (aftanlærisáhættan), hvert mæligildi endar við leikálag, og vikum er haldið svo bráðaálag rjúki ekki upp (ACWR er aðeins umdeilt viðmið). Opnaðu Tímabilsskipulag → Leikmenn → Uppbyggingar-áætlun.",
    },

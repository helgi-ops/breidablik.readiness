...
- Two passes, in order:
  1. **Pass 1 `auto_fill`** (route TS): for every non-OFF team with ≥1 real submission that day,
     insert the **team average** for each player with no row yet (`source='auto_fill'`, `is_imputed=false`).
  2. **Pass 2 `imputed`** (`fn_impute_team_day` → `fn_impute_missing_rpe`): GPS-trained players
     (`player_external_load_daily.total_distance > 0`) that Pass 1 did **not** cover get a per-player
...
declare
  ...
  imp jsonb;
begin
  -- Pass 1: auto_fill players on submitting non-OFF teams who have no row yet.
...
- `pass1_autofill` / `pass2_rpe_imputed` are informational (how much each pass would fill). A `pass2 = 0`
  night just means Pass 1 already covered every GPS player — expected on days most players submit.
...
- **2026-08-30** — target `2026-08-30`; `pass1_autofill=20`, `pass2_rpe_imputed=0`,
  `imputed_collisions=0`, `any_dup_playerdays=0`; 0 rows leaked. Clean.
